...
| 区分 | 対応 | 理由 |
| --- | --- | --- |
| 今すぐ直す | 仕様文中で `Firebase` / `GCS` を採用確定として書かない。旧命名カラムには「旧命名」「ADR待ち」を明記する。 | ADR 0001 の AWS 統一方針と衝突させないため。 |
| 後続ADR待ち | 物理カラム名（`firebase_uid`, `*_gcs_key`）の一括リネーム。外部連携キー命名（`object_key` など）の最終確定。 | 認証方式/オブジェクトストレージ方式が未決定のため。 |

> 注: 本書の旧命名は移行対象の論理名として保持しており、クラウド採用確定を意味しません。
